...
class acousticTextLSTM_CNN_FFNN_Attention():
    ...
    def __init__(self, seq_acoustic_train, seq_acoustic_dev, seq_acoustic_test, glob_acoustic_train, glob_acoustic_dev, glob_acoustic_test, text_train, text_dev, text_test, y_train, y_dev, y_test, tokenizer, csv_path, checkpoint_path, plot_path, class_weights):
        ...
        lstm = layers.LSTM(300, activation='relu', dropout=0.05, recurrent_dropout=0.2, return_sequences=True)(seq_acoustic)
        self.attention = layers.Attention(use_scale=True)([lstm, lstm])
        attention = layers.GlobalMaxPooling1D()(self.attention)

        ffnn = layers.Dense(300)(glob_acoustic)

        fused = layers.Concatenate()([embedded, attention, ffnn])
