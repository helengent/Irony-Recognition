...
class acousticTextLSTM_CNN_FFNN():
    ...
    def __init__(self, seq_acoustic_train, seq_acoustic_dev, seq_acoustic_test, glob_acoustic_train, glob_acoustic_dev, glob_acoustic_test, text_train, text_dev, text_test, y_train, y_dev, y_test, tokenizer, csv_path, checkpoint_path, plot_path, class_weights):
        ...
        embeddingConvs = list()
        for pooling_size, conv_size in zip([4, 6], [104, 96]):
        ...
        embedded = layers.Dense(36, activation='relu')(embedded)

        lstm = layers.LSTM(128, activation='relu', dropout=0.1, recurrent_dropout=0.05)(seq_acoustic)
        lstm = layers.Dense(36, activation='relu')(lstm)

        ffnn = layers.Dense(36)(glob_acoustic)
        ...
        z = layers.Dense(8, activation='relu')(x.output)
