...
class acousticTextCNN_FFNN():


    def __init__(self, glob_acoustic_train, glob_acoustic_dev, glob_acoustic_test, text_train, text_dev, text_test, y_train, y_dev, y_test, tokenizer, csv_path, checkpoint_path, plot_path, class_weights):

        # Load in training, dev, and test data
        self.train_input = [text_train, glob_acoustic_train]
        self.dev_input = [text_dev, glob_acoustic_dev]
        self.test_input = [text_test, glob_acoustic_test]
        ...
        fused = layers.Concatenate()([embedded, ffnn])
        
        dropout = layers.Dropout(0.5)(fused)
        x = keras.Model(inputs=[text, glob_acoustic], outputs=dropout)
