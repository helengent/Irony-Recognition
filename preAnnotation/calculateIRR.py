...
def parseFileNames(directory):
    ...
    fileList = [item.strip(directory) for item in fileList]
    annotators = list(set([item.split('/')[0] for item in fileList]))
    unqFiles = list(set([item.split("_")[1].split("-")[0] for item in fileList]))
    ...
    for a in annotators:
        ...
        for f in newF:
            r = ''
            for fl in fileList:
                if fl[:-6] == "{}/SBsamp_{}".format(a, f):
                    r = fl[-5].upper()
            dfDict[a].append(r)

    iList, nList = list(), list()
    for i, f in enumerate(newF):
        inQ = [fl[-5].upper() for fl in fileList if fl[12:-6] == f]
        N = inQ.count("N")
        I = inQ.count("I")
        iList.append(I)
        nList.append(N)
